...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const personagemData = await window.electronAPI.requestPersonagemData();
        ...
        window.habilidadesData = await window.electronAPI.requestHabilidadesData();
        console.log('Dados das habilidades recebidos:', window.habilidadesData);

        // Exibe as habilidades na interface
        exibirHabilidades(window.habilidadesData);
...
class HabilidadeBase {
    ...
    mostrarMensagem(mensagem) {
        window.api.messageMain(mensagem);
    }
...
class LuciferPassiva extends HabilidadeBase {
    ...
    aplicarAtaque() {
        ...
        danoTotal += resultado;
        this.mostrarMensagem(`Ataque recebido/causado: ${dados} => +${resultado} de dano (Máximo: ${maximo})`);
    }

    aplicarNocaute() {
        let [dados, resultado, maximo] = this.rolarDado(8, 1);
        danoTotal += resultado;
        this.mostrarMensagem(`Inimigo Morto/Aliado Caido: ${dados} => +${resultado} de dano (Máximo: ${maximo})`);
    }

    aplicarBuffDebuff() {
        let [dados, resultado, maximo] = this.rolarDado(6, 2);
        danoTotal += resultado;
        this.mostrarMensagem(`Buff/Debuff em si: ${dados} => +${resultado} de dano (Máximo: ${maximo})`);
    }

    aplicarHabilidade() {
        let [dados, resultado, maximo] = this.rolarDado(8, 2);
        danoTotal += resultado;
        this.mostrarMensagem(`Habilidade inimiga/aliada: ${dados} => +${resultado} de dano (Máximo: ${maximo})`);
    }

    aplicarCriticos() {
        let [dados, resultado, maximo] = this.rolarDado(10, 1);
        danoTotal += resultado;
        this.mostrarMensagem(`Críticos (Aliados ou inimigo): ${dados} => +${resultado} de dano (Máximo: ${maximo})`);
    }

    aplicarAliadoMorto() {
        danoTotal += 30;
        this.mostrarMensagem("Aliado Morto (morte permanente): +30 de dano");
    }
}
...
class Belzebub extends HabilidadeBase {
    ...
    aplicarHabilidade() {
        ...
        this.mostrarMensagem(`Teste de Presença - Manifestação: ${dadosPresenca} => maior dado: ${maiorDado}`);
...
function atualizarEnergia(custo, cooldown) {
    ...
    window.api.messageMain(`Energia restante após gastar ${custo} de energia. Numero de circulos: ${cooldown}`);
}
...
function ajustarEnergia(multiplicador) {
    ...
    if (isNaN(valorAjuste)) {
        window.api.messageMain("Digite um valor válido para o ajuste de energia");
        return;
    }
    if (multiplicador === 1) {
        ...
        window.api.messageMain("Operação inválida para ajuste de energia");
        return;
    }
...
function ajustarSanidade(multiplicador) {
    ...
    if (isNaN(valorAjuste)) {
        window.api.messageMain("Digite um valor válido para o ajuste de sanidade");
        return;
    }
    if (multiplicador === 1) {
        ...
        window.api.messageMain("Operação inválida para ajuste de sanidade");
        return;
    }
...
function ajustarVida(multiplicador) {
    ...
    if (isNaN(valorAjuste)) {
        window.api.messageMain("Digite um valor válido para o ajuste de vida");
        return;
    }
    if (multiplicador === 1) {
        ...
        window.api.messageMain("Operação inválida para ajuste de vida");
        return;
    }
...
function rolarDadosCalculo(atributo, pericia, numeroVantagens, modificador) {
    ...
    if (atributo < 0) {
        ...
        window.api.messageMain(`Dados rolados: ${dadosAtributo.join(', ')}`);

        // Mostra um alerta com o menor dado rolado
        window.api.messageMain(`Menor dado rolado: ${menorDado}`);
        ...
        window.api.messageMain(`Somado no dado final: ${pericia} (perícia) + ${modificador} (modificador) + ${complemento} (complemento)`);
        ...
        window.api.messageMain(`Maior dado rolado: ${maiorDado}`);
...
function executarAcao() {
    ...
    // Exibir o resultado em um window.api.messageMain
    window.api.messageMain(`Resultado da Ação: ${resultado}`);
}
...
function executarAtaque() {
    ...
    // Exibir o resultado em um window.api.messageMain
    window.api.messageMain(`Resultado do Ataque: ${resultado}`);
}
...
// Função para salvar o status do personagem
async function salvarStatus() {
    ...
    // Salvar os dados atualizados através do Electron
    try {
        const status = await window.electronAPI.savePersonagemData(updatedData);
        if (status === 'success') {
            // Exibir mensagem de sucesso ou atualizar interface
            window.api.messageMain('Status do personagem salvo com sucesso!');
        } else {
            // Tratar erro ao salvar
            window.api.messageMain('Erro ao salvar status do personagem.');
        }
    } catch (error) {
        console.error('Erro ao salvar status do personagem:', error);
        window.api.messageMain('Erro ao salvar status do personagem.');
    }
}
...
async function carregarStatus(){
    ...
    window.api.messageMain('Carregamento de Status e Pericias Efetuado !');
}